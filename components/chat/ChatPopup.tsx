...
setTimeout(async () => {
  ...
  await loadMessages();
}, 1200); // delay 1.2s cho giống người thật
      setMessage("");
      await loadMessages();
    } catch (error) {
  ...
  flex h-[750px] w-[480px] flex-col
  overflow-hidden rounded-[22px] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.28)]"
>
  {/* HEADER */}
  <div className="relative shrink-0 bg-[linear-gradient(180deg,#cf2c97_0%,#c43ca1_100%)] px-4 pb-6 pt-5 text-white">
    <button
      type="button"
      onClick={onClose}
      className="absolute right-4 top-4 text-[26px] leading-none text-white/80 transition hover:text-white"
    >
      ×
    </button>
            ...
            return (
              ...
              >
                <div className="max-w-[290px]">
                  ...
                  <div
                    className={`rounded-[20px] px-5 py-4 text-[18px] leading-[1.5] shadow-sm ${
                      isBot
                        ? "bg-[#efe8ed] text-[#111]"
    ...
    <div className="relative shrink-0 border-t border-[#dddddd] bg-white px-3 py-3">
        ...
        <div className="absolute bottom-[62px] right-12 z-20 grid grid-cols-5 gap-3 rounded-[18px] bg-white p-4 shadow-[0_18px_40px_rgba(0,0,0,0.2)]">
          {emojiList.map((emoji) => (
            <button
              ...
              className="text-[28px] leading-none transition hover:scale-110"
            >
              {emoji}
            </button>
          ))}
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          className="flex-1 border-none bg-transparent text-[18px] text-[#444] outline-none placeholder:text-[#9c9c9c]"
        />

        <button
          type="button"
          onClick={() => setShowEmojiPicker((prev) => !prev)}
          className="text-[28px] leading-none text-[#7d7d7d] transition hover:opacity-80"
          aria-label="Mở emoji"
        >
          ☺
        </button>
